...
//problem-1
const id33=inventory.reduce((acc,car)=> car.id===33 ? acc=`Car 33 is a ${car.car_year} ${car.car_make} ${car.car_model}`:acc,'')
console.log(id33)

//problem-2
const lastcar=inventory.reduce((acc,car)=>car.id===inventory.length ? `Last car is a ${car.car_year} ${car.car_make} ${car.car_model}`:acc,'')
console.log(lastcar)

//problem-3
const sortalpha=inventory
.map(car=>car.car_model.toLowerCase())
.sort();
console.log(sortalpha)

//problem-4
const years=inventory.reduce((acc,car)=>{
    if (!acc[car.car_year]){
        acc[car.car_year]=[];
    }
    acc[car.car_year].push(`${car.car_make} ${car.car_model}`)
    return acc;
},{})
console.log(years)

//problem-5
const makeyears=inventory.reduce((acc,car)=>{
    if(!acc.includes(car.car_year)){
        acc.push(car.car_year)
    }
    return acc
},[])
console.log(makeyears)

//problem-6
const oldercars = inventory.reduce((acc, car) => {
    if (!acc.includes(car.car_year) && car.car_year < 2000) {
      acc.push(car.car_year);
    }
    return acc;
}, []);
  
console.log(oldercars, oldercars.length);
